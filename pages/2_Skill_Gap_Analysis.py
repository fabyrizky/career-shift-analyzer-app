...
if st.button("🔎 Lihat Analisis Skill Gap", type="primary"):
    if skills_user.strip():
        ...
        # Calculate matches - FIXED SYNTAX
        ...
        tools_match = [s for s in user_skills if s in [skill.lower() for skill in field_data["tools"]]]  # FIXED THIS LINE
        ...
        with col1:
            ...
            if not technical_match and not soft_match and not tools_match:
                st.info("Belum ada skill yang match. Mari mulai belajar! 💪")
        ...
        # Radar chart - IMPROVED WITH ERROR HANDLING
        if total_required > 0:
            ...
            try:
                ...
                current_scores = [
                    (len(technical_match) / len(field_data["technical"]) * 100) if len(field_data["technical"]) > 0 else 0,
                    (len(soft_match) / len(field_data["soft"]) * 100) if len(field_data["soft"]) > 0 else 0,
                    (len(tools_match) / len(field_data["tools"]) * 100) if len(field_data["tools"]) > 0 else 0
                ]
                ...
                st.info("Radar chart tidak dapat ditampilkan, tapi analisis tetap valid.")
        ...
        if priority_skills:
            ...
            for i, skill in enumerate(priority_skills[:8], 1):  # Show max 8 recommendations
                resource_url = learning_resources.get(skill.lower(), f"https://www.coursera.org/search?query={skill.replace(' ', '%20')}")
        ...
        if skill_percentage >= 80:
            st.success("🌟 **Excellent!** You're well-prepared for this field. Consider specializing in advanced areas.")
        elif skill_percentage >= 60:
            st.info("👍 **Good progress!** Focus on the missing technical skills to become job-ready.")
        elif skill_percentage >= 40:
            st.warning("⚡ **Getting there!** You have a solid foundation. Time to accelerate your learning.")
        else:
            st.error("🚀 **Start strong!** This is a great opportunity to build expertise from the ground up.")
        
        # Career readiness assessment
        st.subheader("🎯 Career Readiness Assessment")
        
        readiness_factors = {
            "Technical Foundation": len(technical_match) / len(field_data["technical"]) if len(field_data["technical"]) > 0 else 0,
            "Soft Skills": len(soft_match) / len(field_data["soft"]) if len(field_data["soft"]) > 0 else 0,
            "Tools Proficiency": len(tools_match) / len(field_data["tools"]) if len(field_data["tools"]) > 0 else 0
        }
        
        for factor, score in readiness_factors.items():
            percentage = score * 100
            st.write(f"**{factor}:** {percentage:.1f}%")
            st.progress(score)
